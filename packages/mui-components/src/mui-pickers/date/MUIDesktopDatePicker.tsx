...
type DesktopDatePickerValue = Parameters<
  NonNullable<DesktopDatePickerProps['onChange']>
>[0];

type PickerOnValueChangeProps<ValidationError> = {
  newValue: DesktopDatePickerValue;
  context: PickerChangeHandlerContext<ValidationError>;
};

export type MUIDesktopDatePickerProps = {
  /**
...
const MUIDesktopDatePicker = forwardRef(function MUIDesktopDatePicker(
  {
    fieldName,
    value: muiValue,
    onValueChange,
    ...
    customIds,
    ...otherPickerProps
  }: MUIDesktopDatePickerProps,
  ref: Ref<HTMLInputElement>
) {
  const { dateAdapter, allLabelsAboveFields } = useContext(MUIComponentsConfigContext);
  if (!dateAdapter) {
    throw new Error(generateDateAdapterErrMsg('MUIDesktopDatePicker'));
  }
