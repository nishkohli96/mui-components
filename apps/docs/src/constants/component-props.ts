...
export const PropsDescription: Record<string, PropsInfo> = Object.freeze({
  ...
  errorMessage: {
    name: 'errorMessage',
    description:
      'Error message for the field. Any non-empty string puts the field into an error state and is surfaced through the helper text. Pass `undefined` or `null` to clear the error state.',
    type: 'string | null'
  },
  error: {
    name: 'error',
    description:
      'Forces the field\'s error state regardless of `errorMessage` — useful when a form library reports an error without a message string. When omitted, the error state is derived from `errorMessage`.',
    type: 'boolean'
  },
  renderError: {
    name: 'renderError',
    description:
      'Custom renderer for `errorMessage`. Receives the raw error string and must return renderable content, e.g. wrapping it with an icon or a styled element.',
