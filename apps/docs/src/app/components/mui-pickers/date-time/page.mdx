...
export const metadata = {
  ...
  description: 'Controlled Material UI X date-time pickers — responsive, desktop, mobile and static variants with label, error and helper-text handling.'
};
...
These components wrap the Material UI X [Date-time Picker](https://v7.mui.com/x/react-date-pickers/date-time-picker/) with the
...
Responsive, desktop and mobile variants (`MUIDateTimePicker`, `MUIDesktopDateTimePicker`,
`MUIMobileDateTimePicker`) share the props below. Props marked with <strong style={{ color: 'var(--mui-palette-error-main)' }}>{'*'}</strong> are required.

<PropsTable rows={ComponentProps.MUIDateTimePicker} />
...
<PropsTable rows={ComponentProps.MUIStaticDateTimePicker} />
